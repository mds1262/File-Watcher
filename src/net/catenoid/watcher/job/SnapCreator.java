...
public class SnapCreator {
	...
	public SnapCreator(Config config, String jobName, FileItem file)
	{
		this.conf = config;
		this.fileitem = file;
...
//		log.debug("SnapCreator - run");
		...
		float _fsecond;
		if(fileitem.mediaInfo.videoDuration == null){
			_fsecond = Float.parseFloat(fileitem.mediaInfo.duration) / 1000;
		} else {
			_fsecond = Float.parseFloat(fileitem.mediaInfo.videoDuration) / 1000;
		}
		...
		if(fileitem.getPoster() != null && fileitem.getPoster().getPosition() != -1) {
			...
			custom_info = conf.getSnap().get_custom_snapshot(fileitem.content_provider_key);
		}
		...
		String startOffset = myUtils.secondsToString(second);	
		...
		if(isSimulator() == false) {
			...
			String temp = String.format("%s", fileitem.get_snapshot_path());
...
//		}
		...
		
		{
			...
			snapDir.mkdirs();
			myUtils.chmod777(snapDir.getAbsolutePath());
		}
...
//			}
			...
			long _end = System.currentTimeMillis();
			log.debug(String.format("filename: %s , dur[%s]", fileitem.get_physical_path(), myUtils.millisToDHMS(_end - _start)));
    ...
    public String scaleString(Poster custom_info){
    	int org_media_width=Integer.valueOf(fileitem.mediaInfo.videoWidth);
    	int org_media_height=Integer.valueOf(fileitem.mediaInfo.videoHeight);
...
//    	}
    ...
    public String scaleStringExt(Poster custom_info){
    	...
    	int transpose=0;    	
    	float rotation=Float.valueOf(fileitem.mediaInfo.getRotation());
