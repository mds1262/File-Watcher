...
public class ExtFilenameParserDTO {
	...
	private ExtFilenameParserDTO() {
	}
	
	public static ExtFilenameResultDTO newParser(String path) {
		ExtFilenameParserDTO parser = new ExtFilenameParserDTO();
		parser.parser(path);	
		
		ExtFilenameResultDTO result = new ExtFilenameResultDTO(parser);
		return result;
	}
...
//			throw new FilenameParserException("not exist filename: " + fullPath);
		...
		if(isReservedPath) {
			...
			if(checkPassThroughFolder(paths.get(2))) {
				...
				if(sep == -1) {
					...
					log.debug("not exist passthrough === "+sep);
				} else {
					if (dot != -1) {
						strMediaProfileKey = strFilename.substring(sep + 1, dot);
						strTitle = strTitle.substring(0, strTitle.length() - (strMediaProfileKey.length()+1));
						int idx2 = strMediaProfileKey.indexOf("+");
						// Passthrough 선진행 관련 Profile 획득 추가
						if(idx2 > -1) {
							strMediaProfileKey = strMediaProfileKey.substring(0,idx2);
						}else {
							// 고객의 실수로인한 방어코드
							int atIdx = strMediaProfileKey.indexOf("@");
							
							if(atIdx > -1) {
								strMediaProfileKey.substring(0, atIdx);
							}
						}
					} else {
						strMediaProfileKey = strFilename.substring(sep + 1);
						strTitle = strTitle.substring(0, strTitle.length() - (strMediaProfileKey.length()+1));
					}
				}
			}
		 ...
		 */
		strUploadPath = WatcherUtils.implodeList(paths, "/");
	}
	...
	public String media_profile_key() {
		if(WatcherUtils.isEmpty(strMediaProfileKey)) {
			return null;
		}
